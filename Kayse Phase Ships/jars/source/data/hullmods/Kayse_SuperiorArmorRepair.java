...
public class Kayse_SuperiorArmorRepair extends BaseHullMod {
    
    public static Logger log = Global.getLogger(Kayse_ArmorRepair.class);

    private static final float ARMOR_REPAIR_MULTIPLIER = 400.0f/15f;
//Repairing the armor cell approximately one Shepherd's cell's worth (200 armor/15 cells) for Capital ships
    private static final float MAX_ARMOR_RECOVERED = 0.75f;
    private static final float FLUX_COST_TO_REPAIR = 10f;
    private static final float FLUX_CUTOFF = 0.75f;
    
    public static float PHASE_COOLDOWN_SLOWDOWN = 150f;
...
//            cellLoc.y += cellSize * 0.5f - cellSize * (float) Math.random();
            MagicRender.objectspace(Global.getSettings().getSprite("fx","kayse_hex"),
                    ship,
                    //cellLoc,
...
//                    AFTER_COLOR);
    ...
    public String getDescriptionParam(int index, HullSize hullSize) {
        if (index == 0) return "" + (int) 15;//15 seconds is a wag.
    ...
    public boolean isApplicableToShip(ShipAPI ship) {
        if (! ship.getVariant().hasHullMod("phasefield"))
            return false;
        if (ship.getVariant().hasHullMod("kayse_armorrepair"))
            return false;
